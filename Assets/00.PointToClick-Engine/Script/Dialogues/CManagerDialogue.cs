    ...
    public class CManagerDialogue : VariableStorageBehaviour
    {
      ...
      //  private DialogueSaver dialogueSaver;
        ...
        public string FindNodeReturnNodeAcrossAllYarnProjects(string nodeNameToFind)
        {
            ...
            {
                // Search in Point and Click Yarn Projects
                foreach (YarnProject yarnProject in ListYarnDialogueText)
                {
                    foreach (string nodeName in yarnProject.NodeNames)
                    {
                        if (nodeName == nodeNameToFind)
                        {
                            Debug.Log("Nodo encontrado en Point and Click Yarn Project: " + nodeNameToFind);
                            return nodeName;
                        }
                    }
                }

                // Search in Visual Novel Yarn Projects
                foreach (YarnProject yarnProject in ListYarnDialogueVisualNovel)
                {
                    foreach (string nodeName in yarnProject.NodeNames)
                    {
                        if (nodeName == nodeNameToFind)
                        {
                            Debug.Log("Nodo encontrado en Visual Novel Yarn Project: " + nodeNameToFind);
                            return nodeName;
                        }
                    }
                }

                Debug.Log("NO ENCUENTRA NODO en ningún Yarn Project: " + nodeNameToFind);
            }
            else
            {
            ...
            return null; // Return null if not found in any project
        }
        ...
        public void StartDialogueRunnerAcrossAllYarnProjects(string dialogueName)
        {
            if (dialogueRunner != null)
            {
                // 1. Find the node across all projects:
                string foundNode = FindNodeReturnNodeAcrossAllYarnProjects(dialogueName);

                if (foundNode != null)
                {
                    // 2. Check dialogue history (optional, but recommended for preventing repeats):
